...
const annotationTypes = {
  ...
  question: {
    ...
    label: 'Question',
    color: '#ff9800',
    description: 'Question or uncertainty',
  },
  concern: {
    icon: ConcernIcon,
    label: 'Concern',
    color: '#f44336',
    description: 'Area of concern or flag',
  },
  observation: {
  ...
  hypothesis: {
    ...
    label: 'Hypothesis',
    color: '#9c27b0',
    description: 'Hypothesis or theory',
  },
}
...
export default function AnnotationTools({
  ...
  const handleSaveAnnotation = async () => {
    try {
      ...
      if (editingAnnotation) {
        ...
        setAnnotations((prev) =>
          prev.map((a) => (a.id === editingAnnotation.id ? annotation : a))
        )
  ...
  const getRegionName = (regionId: string) => {
    ...
    return region
      ? `${region.spatial_location} (${(region.importance_score * 100).toFixed(0)}% importance)`
      : regionId
  }
  ...
  const renderAnnotationItem = (annotation: Annotation) => {
    ...
    return (
      <Card
        key={annotation.id}
        sx={{ mb: 1, border: `1px solid ${typeConfig.color}` }}
      >
        <CardContent sx={{ pb: 1 }}>
        ...
        <Box>
          {annotations.length > 0 ? (
            <Box>
              {/* Group by region */}
              {regions.map((region) => {
                const regionAnnotations = getAnnotationsByRegion(
                  region.region_id
                )
              ...
              {annotations
                .filter(
                  (a) => !regions.some((r) => r.region_id === a.region_id)
                )
            ...
            <Paper sx={{ p: 3, textAlign: 'center' }}>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                Add notes, questions, or observations to help with
                interpretation
              </Typography>
              {!readOnly && (
                <Button
      ...
      >
            ...
            <FormControl fullWidth sx={{ mb: 3 }}>
              ...
              <Select
                ...
                onChange={(e) =>
                  setNewAnnotation((prev) => ({
                    ...prev,
                    type: e.target.value as keyof typeof annotationTypes,
                  }))
                }
                label="Type"
              >
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              !newAnnotation.text.trim() || !newAnnotation.region_id || loading
            }
