...
def iam_role_arn(draw):
    ...
    role_name = draw(st.text(min_size=1, max_size=64, alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_"))
...
def s3_encryption_config(draw):
    ...
    if algorithm == "aws:kms":
        config["Rules"][0]["ApplyServerSideEncryptionByDefault"]["KMSMasterKeyID"] = (
...
class TestSecurityConfigurationEnforcement:
    ...
    @given(
        ...
        policy_doc=iam_policy_document()
    )
    ...
    @given(
        ...
        encryption_config=s3_encryption_config()
    )
    @hypothesis_settings(max_examples=50, deadline=5000)
    def test_s3_encryption_enforcement(self, bucket_name, encryption_config):
        ...
        if has_valid_encryption:
            ...
            if any(rule.get('ApplyServerSideEncryptionByDefault', {}).get('SSEAlgorithm') == 'invalid-algorithm' 
                   for rule in rules):
        ...
        if not has_valid_encryption:
            ...
            has_invalid_algorithm = any(
                rule.get('ApplyServerSideEncryptionByDefault', {}).get('SSEAlgorithm') == 'invalid-algorithm'
                for rule in rules
            )
            ...
            if has_invalid_algorithm:
                ...
                assert any("unsupported encryption algorithm" in violation.lower() for violation in result.violations), (
                ...
                assert any("encryption" in violation.lower() for violation in result.violations), (
                    "Missing encryption should generate encryption-related violations"
                )
        else:
            # Should be compliant if encryption is properly configured
            encryption_violations = [v for v in result.violations if "encryption" in v.lower()]
    ...
    def test_security_group_minimal_exposure(self, sg_ids, ingress_rules):
        ...
        if has_open_ssh:
            ...
            assert any("SSH" in violation or "22" in violation for violation in result.violations), (
        ...
        if has_open_all_ports:
            ...
            assert any("all traffic" in violation.lower() for violation in result.violations), (
    ...
    def test_vpc_private_subnet_requirement(self, vpc_id):
        ...
        if not has_private_subnets:
            ...
            assert any("private subnet" in violation.lower() for violation in result.violations), (
    ...
    def test_encryption_in_transit_enforcement(self):
        ...
        if settings.is_production:
            assert result.details['production_https_required'] == True, (
    ...
    def test_comprehensive_security_audit(self, role_arn, bucket_names, sg_ids, vpc_id):
        ...
        valid_buckets = [
            ...
            if len(bucket) >= 3 and not bucket.startswith('-') and not bucket.endswith('-')
        ]
        ...
        expected_components = [
            'iam_role', 's3_buckets', 'security_groups', 
            'vpc', 'encryption_in_transit', 'identity'
        ]
    ...
    def _setup_comprehensive_mocks(self, role_arn, bucket_names, sg_ids, vpc_id):
        ...
        self.mock_iam_client.list_attached_role_policies.return_value = {
            'AttachedPolicies': [
                {'PolicyName': 'S3Access', 'PolicyArn': 'arn:aws:iam::aws:policy/AmazonS3ReadOnlyAccess'}
            ]
        }
