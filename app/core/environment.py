...
class EnvironmentConfig(BaseModel):
    ...
    s3_bucket_name: Optional[str] = Field(
        default=None,
        description="S3 bucket name for production storage"
    )
    ...
    metrics_enabled: bool = Field(
        default=False,
        description="Whether usage metrics collection is enabled"
    )
    ...
    static_dir: str = Field(
        default="static",
        description="Local static files directory"
    )
...
class EnvironmentDetector:
    ...
    def detect_environment(cls) -> EnvironmentType:
        ...
        Detection logic:
        1. Check APP_ENVIRONMENT variable explicitly
        2. If AWS_REGION is set, assume production
    ...
    def create_config(cls) -> EnvironmentConfig:
        ...
        # Get feature flags
        metrics_enabled = os.getenv(cls.ENV_VAR_ENABLE_METRICS, "false").lower() in ["true", "1", "yes"]
        secret_manager_enabled = os.getenv(cls.ENV_VAR_ENABLE_SECRETS_MANAGER, "false").lower() in ["true", "1", "yes"]
        ...
        try:
            ...
            logger.info(f"Environment configuration created: {config.environment.value}")
