...
class RateLimiter:
    ...
    def _get_rate_limit_info(self, client_id: str) -> Dict[str, int]:
        ...
        return {
            ...
            "remaining_hour": max(0, self.rule.requests_per_hour - len(self.requests[client_id])),
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp):
        ...
        self.rate_limiters = {
            "default": RateLimiter(RateLimitRule(
                requests_per_minute=60,
                requests_per_hour=1000,
                burst_limit=10
            )),
            "upload": RateLimiter(RateLimitRule(
                requests_per_minute=10,
                requests_per_hour=100,
                burst_limit=3
            )),
            "analysis": RateLimiter(RateLimitRule(
                requests_per_minute=30,
                requests_per_hour=500,
                burst_limit=5
            )),
            "auth": RateLimiter(RateLimitRule(
    ...
    def _validate_request_content(self, request: Request) -> Optional[Response]:
        ...
        for pattern in self.suspicious_patterns:
            if pattern in path or pattern in query:
                logger.warning(f"Blocked suspicious request pattern: {pattern} in {path}")
                return JSONResponse(
                    status_code=status.HTTP_403_FORBIDDEN,
                    content={
                        "error": "Access denied",
    ...
    def _create_rate_limit_response(
        ...
        response.headers["X-RateLimit-Limit-Minute"] = str(rate_limit_info["limit_per_minute"])
        response.headers["X-RateLimit-Limit-Hour"] = str(rate_limit_info["limit_per_hour"])
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        is_testing = (
            request.headers.get("user-agent") == "testclient" or
            "testserver" in str(request.url) or
            hasattr(request.state, "testing") or
