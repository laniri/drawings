...
class ModelTrainingRequest(BaseModel):
    ...
    min_samples: int = Field(50, ge=10, description="Minimum samples required for training")
...
class TrainingConfigRequest(BaseModel):
    ...
    def validate_splits_sum_to_one(cls, v, info):
        ...
        if 'train_split' in info.data and 'validation_split' in info.data and 'test_split' in info.data:
            total = info.data['train_split'] + info.data['validation_split'] + info.data['test_split']
...
class ModelExportRequest(BaseModel):
    ...
    age_group_min: float = Field(..., ge=2.0, le=18.0)
    age_group_max: float = Field(..., ge=2.0, le=18.0)
    export_format: ModelExportFormat = Field(ModelExportFormat.PYTORCH, description="Export format")
...
class EnhancedModelDeploymentRequest(BaseModel):
    ...
    age_group_min: float = Field(..., ge=2.0, le=18.0)
    age_group_max: float = Field(..., ge=2.0, le=18.0)
    replace_existing: bool = Field(False, description="Whether to replace existing model")
    validate_before_deployment: bool = Field(True, description="Validate model before deployment")
    backup_existing: bool = Field(True, description="Backup existing model before replacement")
    deployment_environment: str = Field("production", description="Target deployment environment")
