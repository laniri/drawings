...
class BatchAnalysisRequest(BaseModel):
    ...
    drawing_ids: List[int] = Field(
        ..., min_length=1, max_length=100, description="List of drawing IDs to analyze"
    )
    force_reanalysis: bool = Field(
        False, description="Force re-analysis even if results exist"
    )
...
class AnomalyAnalysisResponse(BaseModel):
    ...
    visual_anomaly_score: Optional[float] = Field(
        None, description="Visual component reconstruction loss (dims 0-767)"
    )
    subject_anomaly_score: Optional[float] = Field(
        None, description="Subject component reconstruction loss (dims 768-831)"
    )
    ...
    analysis_type: str = Field(
        "subject_aware", description="Type of analysis performed"
    )
    subject_category: Optional[str] = Field(
        None, description="Subject category used in analysis"
    )
    is_anomaly: bool = Field(
        ..., description="Whether the drawing is flagged as anomalous"
    )
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence in the anomaly decision"
    )
...
class ComparisonExampleResponse(BaseModel):
    ...
    similarity_score: float = Field(
        ..., ge=0.0, le=1.0, description="Similarity score to the analyzed drawing"
    )
    anomaly_score: float = Field(
...
class BatchAnalysisResponse(BaseModel):
    ...
    total_drawings: int = Field(
        ..., gt=0, description="Total number of drawings to analyze"
    )
    ...
    results: List[AnalysisResultResponse] = Field(
        default_factory=list, description="Completed analysis results"
    )
    errors: List[Dict[str, Any]] = Field(
        default_factory=list, description="Error details for failed analyses"
    )
    started_at: datetime
...
class InteractiveRegionResponse(BaseModel):
    ...
    importance_score: float = Field(
        ..., ge=0.0, le=1.0, description="Importance score for this region"
    )
...
class AttentionPatchResponse(BaseModel):
    ...
    attention_weight: float = Field(
        ..., ge=0.0, le=1.0, description="Attention weight for this patch"
    )
...
class InteractiveInterpretabilityResponse(BaseModel):
    ...
    saliency_regions: List[InteractiveRegionResponse] = Field(
        ..., description="Interactive regions with hover explanations"
    )
    attention_patches: List[AttentionPatchResponse] = Field(
        ..., description="Vision Transformer attention patch data"
    )
    region_explanations: Dict[str, str] = Field(
    ...
    interaction_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Metadata for interactive features"
    )
...
class SimplifiedExplanationResponse(BaseModel):
    ...
    age_appropriate_context: str = Field(
        ..., description="Context appropriate for the age group"
    )
    recommendations: List[str] = Field(
        default_factory=list, description="Simple recommendations"
    )
...
class ConfidenceMetricsResponse(BaseModel):
    ...
    explanation_reliability: float = Field(
        ..., ge=0.0, le=1.0, description="Reliability of the explanation"
    )
    model_certainty: float = Field(
        ..., ge=0.0, le=1.0, description="Model's certainty in the prediction"
    )
    data_sufficiency: str = Field(
        ..., description="Sufficient/Limited/Insufficient data quality"
    )
    warnings: List[str] = Field(
        default_factory=list, description="Confidence-related warnings"
    )
    technical_details: Dict[str, Any] = Field(
        default_factory=dict, description="Technical confidence metrics"
    )
...
class AnnotationRequest(BaseModel):
    ...
    annotation_text: str = Field(
        ..., min_length=1, max_length=500, description="User annotation text"
    )
    ...
    def validate_annotation_type(cls, v):
        ...
        if v.lower() not in allowed_types:
            raise ValueError(
                f'Annotation type must be one of: {", ".join(allowed_types)}'
            )
        return v.lower()
...
class ComparisonExamplesResponse(BaseModel):
    ...
    normal_examples: List[Dict[str, Any]] = Field(
        ..., description="Normal drawings from same age group"
    )
    anomalous_examples: List[Dict[str, Any]] = Field(
