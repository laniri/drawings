...
from app.services.embedding_service import get_embedding_service, VisionTransformerWrapper
...
class AttentionRollout:
    ...
    def generate_rollout(self, input_tensor: torch.Tensor, start_layer: int = 0) -> torch.Tensor:
        """
    ...
    def _compute_rollout(self, start_layer: int = 0) -> torch.Tensor:
        ...
        for attention in self.attention_maps[start_layer:]:
            ...
            _, indices = torch.topk(flat_attention, int(flat_attention.shape[0] * (1 - self.discard_ratio)))
            
            # Create mask for top attention weights
            mask = torch.zeros_like(flat_attention)
            mask[indices] = 1
            attention_heads_fused = attention_heads_fused * mask.view(attention_heads_fused.shape)
            
            # Normalize
            attention_heads_fused = attention_heads_fused / attention_heads_fused.sum(dim=-1, keepdim=True)
            
            # Add residual connection (identity matrix)
            attention_heads_fused = 0.5 * attention_heads_fused + 0.5 * torch.eye(num_tokens)
    ...
    def generate_cam(self, 
        ...
        try:
            ...
            if self.gradients is None or self.activations is None:
                raise SaliencyGenerationError("Failed to capture gradients or activations")
...
class PatchImportanceScorer:
    ...
    def compute_attention_importance(self, 
                                   input_tensor: torch.Tensor,
                                   method: str = "rollout") -> torch.Tensor:
        """
        ...
        try:
            if method == "rollout":
                ...
                with torch.no_grad():
                    ...
                    if hasattr(outputs, 'attentions') and outputs.attentions:
                        ...
                        raise AttentionVisualizationError("No attention weights available")
            else:
            ...
            raise AttentionVisualizationError(f"Importance computation failed: {str(e)}")
    ...
    def reshape_to_spatial(self, 
        ...
        if len(importance_scores) != expected_patches:
            logger.warning(f"Expected {expected_patches} patches, got {len(importance_scores)}")
...
class SaliencyMapGenerator:
    ...
    def generate_saliency_map(self, 
        ...
        try:
            ...
            if method == "attention_rollout":
                importance_scores = self._generate_attention_saliency(image_tensor, vit_model)
                ...
                attention_scores = self._generate_attention_saliency(image_tensor, vit_model)
    ...
    def _generate_gradient_saliency(self, 
                                  image_tensor: torch.Tensor, 
                                  model: ViTModel,
                                  reconstruction_loss: float) -> torch.Tensor:
        """Generate saliency using gradient-based methods."""
        patch_scorer = PatchImportanceScorer(model)
        return patch_scorer.compute_gradient_importance(image_tensor, reconstruction_loss)
    
    def _create_saliency_visualization(self, 
...
class VisualFeatureIdentifier:
    ...
    def identify_important_regions(self, 
                                 saliency_map: np.ndarray,
    ...
    def _describe_spatial_location(self, 
                                 row: int, 
                                 col: int, 
    ...
    def analyze_drawing_content(self, 
                              image: Union[Image.Image, np.ndarray],
                              important_regions: List[Dict]) -> Dict:
        """
...
class ExplanationGenerator:
    ...
    def generate_explanation(self, 
        ...
        try:
            ...
            recommendations = self._generate_recommendations(severity, important_regions)
    ...
    def _generate_main_explanation(self, 
        ...
        templates = self.explanation_templates.get(template_key, self.explanation_templates["low_anomaly"])
    ...
    def _generate_detailed_analysis(self, 
        ...
        if not important_regions:
            analysis_points.append("No specific regions of high importance were identified.")
    ...
    def _generate_recommendations(self, 
                                severity: str, 
                                important_regions: List[Dict]) -> List[str]:
        ...
        if important_regions:
            ...
            if "center" in spatial_location:
                recommendations.append("Pay attention to central elements in future drawings.")
            elif "top" in spatial_location:
                recommendations.append("Monitor upper region elements in subsequent drawings.")
            elif "bottom" in spatial_location:
                recommendations.append("Observe lower region patterns in future assessments.")
    ...
    def _analyze_metadata(self, metadata: Dict, severity: str) -> Dict:
        ...
        if "age_years" in metadata:
            ...
            if age < 5 and severity in ["high", "medium"]:
                insights["age_consideration"] = "Young age may contribute to developmental variations."
            elif age > 12 and severity == "high":
                insights["age_consideration"] = "Older age makes significant deviations more noteworthy."
        ...
        if "subject" in metadata and metadata["subject"]:
            ...
            if "person" in subject or "human" in subject:
                insights["subject_insight"] = "Human figure drawings can reveal developmental patterns."
            elif "house" in subject:
                insights["subject_insight"] = "House drawings often reflect spatial understanding."
            elif "tree" in subject:
                insights["subject_insight"] = "Tree drawings can indicate emotional expression."
        ...
        if "expert_label" in metadata and metadata["expert_label"]:
            ...
            if label == "concern" and severity in ["low", "minimal"]:
                insights["expert_comparison"] = "AI assessment differs from expert concern - review recommended."
...
class ImportanceRegionDetector:
    ...
    def detect_bounding_boxes(self, 
                            saliency_map: np.ndarray,
                            threshold: float = 0.7,
                            min_region_size: int = 10) -> List[Dict]:
        """
        ...
        try:
            ...
            if measure is not None:
                ...
                region_props = measure.regionprops(labeled_regions, intensity_image=saliency_map)
            else:
                ...
                for label in range(1, num_labels + 1):
                    ...
                    if len(coords[0]) > 0:
                        ...
                        region_prop = type('RegionProp', (), {
                            'label': label,
                            'area': np.sum(mask),
                            'bbox': (coords[0].min(), coords[1].min(), coords[0].max() + 1, coords[1].max() + 1),
            ...
            for prop in region_props:
                ...
                bbox_info = {
                    ...
                    "aspect_ratio": (max_col - min_col) / (max_row - min_row) if max_row > min_row else 1.0
                }
    ...
    def create_region_highlights(self, 
                               original_image: Union[Image.Image, np.ndarray],
                               bounding_boxes: List[Dict],
                               save_path: Optional[str] = None) -> Union[Image.Image, str]:
        """
        ...
        try:
            ...
            for i, bbox_info in enumerate(bounding_boxes[:4]):  # Limit to top 4 regions
                ...
                # Draw label text
                draw.text((min_col, min_row - 20), label, fill=(255, 255, 255), font=font)
...
class SaliencyOverlayGenerator:
    ...
    def create_contour_overlay(self, 
        ...
        try:
            ...
            # Draw contours
            cv2.drawContours(image_cv, contours, -1, contour_color[::-1], line_width)  # BGR format
    ...
    def create_masked_overlay(self, 
                            original_image: Union[Image.Image, np.ndarray],
                            saliency_map: np.ndarray,
                            threshold: float = 0.6,
    ...
    def create_side_by_side_comparison(self, 
        ...
        try:
            ...
            if overlay_type == 'heatmap':
                overlay_image = self.create_heatmap_overlay(pil_image, saliency_map, **overlay_kwargs)
            elif overlay_type == 'contour':
                overlay_image = self.create_contour_overlay(pil_image, saliency_map, **overlay_kwargs)
            elif overlay_type == 'masked':
                overlay_image = self.create_masked_overlay(pil_image, saliency_map, **overlay_kwargs)
            else:
            ...
            draw.text((width + 10, 10), f"Saliency ({overlay_type})", fill=(0, 0, 0), font=font)
            
            return comparison_image
            
        except Exception as e:
            logger.error(f"Failed to create side-by-side comparison: {str(e)}")
            raise SaliencyGenerationError(f"Side-by-side comparison creation failed: {str(e)}")
...
class VisualizationExporter:
    ...
    def _create_report_layout(self, 
        ...
        # Add title
        draw.text((25, 10), "Anomaly Detection Analysis Report", fill=(0, 0, 0), font=title_font)
        ...
        draw.text((25, y_pos), f"Severity: {severity.title()}", fill=(0, 0, 0), font=text_font)
        y_pos += 15
        draw.text((25, y_pos), f"Anomaly Score: {anomaly_score:.3f}", fill=(0, 0, 0), font=text_font)
        y_pos += 15
        draw.text((25, y_pos), f"Normalized Score: {normalized_score:.3f}", fill=(0, 0, 0), font=text_font)
    ...
    def export_interactive_data(self, 
                              saliency_result: Dict,
                              explanation: Dict,
                              filename: str) -> str:
        """
    ...
    def export_presentation_slides(self, 
        ...
        try:
            ...
            slide2 = self._create_analysis_slide(heatmap_overlay, "Attention Heatmap", explanation)
            ...
            slide3 = self._create_analysis_slide(contour_overlay, "Important Regions", explanation)
    ...
    def _create_title_slide(self, 
        ...
        draw.text((50, 520), f"Severity: {severity.title()}", fill=severity_color, font=subtitle_font)
        
        return slide
    
    def _create_analysis_slide(self, 
        ...
        draw.text((50, y_pos), f"Anomaly Score: {anomaly_score:.3f}", fill=(0, 0, 0), font=text_font)
        draw.text((300, y_pos), f"Normalized Score: {normalized_score:.3f}", fill=(0, 0, 0), font=text_font)
...
class InterpretabilityPipeline:
    ...
    def generate_complete_analysis(self, 
                                 ...
                                 export_options: Optional[Dict] = None) -> Dict:
        """
        ...
        try:
            ...
            visualizations['heatmap_overlay'] = self.overlay_generator.create_heatmap_overlay(
                image, saliency_result['saliency_map']
            )
            
            visualizations['contour_overlay'] = self.overlay_generator.create_contour_overlay(
                image, saliency_result['saliency_map']
            )
            
            visualizations['side_by_side'] = self.overlay_generator.create_side_by_side_comparison(
                image, saliency_result['saliency_map'], overlay_type='heatmap'
            )
            
            # Create highlighted regions
            if bounding_boxes:
                visualizations['highlighted_regions'] = self.region_detector.create_region_highlights(
            ...
            if export_options:
                ...
                if export_options.get('export_report', False):
                    exported_files['report'] = self.exporter.export_comprehensive_report(
                        image, saliency_result, explanation, base_filename
                    )
                
                if export_options.get('export_interactive', False):
                    exported_files['interactive_data'] = self.exporter.export_interactive_data(
...
class SaliencyOverlayGenerator:
    ...
    def create_contour_overlay(self, 
        ...
        try:
            ...
            cv2.drawContours(image_with_contours, contours, -1, contour_color, line_width)
    ...
    def create_side_by_side_comparison(self, 
                                     original_image: Union[Image.Image, np.ndarray],
                                     saliency_map: np.ndarray,
                                     overlay_type: str = 'heatmap') -> Image.Image:
        """
        Create side-by-side comparison of original and overlay.
        ...
        try:
            ...
            # Create overlay based on type
            if overlay_type == 'heatmap':
                overlay_image = self.create_heatmap_overlay(original_image, saliency_map)
            elif overlay_type == 'contour':
                overlay_image = self.create_contour_overlay(original_image, saliency_map)
            elif overlay_type == 'masked':
            ...
            draw.text((10, 10), "Original", fill=(0, 0, 0), font=font)
            draw.text((width + 20, 10), f"Saliency ({overlay_type})", fill=(0, 0, 0), font=font)
...
class VisualizationExporter:
    ...
    def export_visualization_set(self, 
                               original_image: Union[Image.Image, np.ndarray],
                               saliency_map: np.ndarray,
                               bounding_boxes: List[Dict],
                               explanation: Dict,
        ...
        try:
            ...
            visualizations = {
                'heatmap': overlay_generator.create_heatmap_overlay(original_image, saliency_map),
                'contour': overlay_generator.create_contour_overlay(original_image, saliency_map),
                'masked': overlay_generator.create_masked_overlay(original_image, saliency_map),
                'side_by_side': overlay_generator.create_side_by_side_comparison(original_image, saliency_map),
                'bounding_boxes': region_detector.create_region_highlights(original_image, bounding_boxes)
            }
            ...
            if 'png' in formats:
                report_path = self._create_summary_report(
                    original_image, saliency_map, bounding_boxes, 
                    explanation, base_filename
                )
    ...
    def _create_summary_report(self, 
        ...
        try:
            ...
            heatmap = overlay_generator.create_heatmap_overlay(original_image, saliency_map)
            bbox_image = region_detector.create_region_highlights(original_image, bounding_boxes)
            ...
            summary_image = Image.new('RGB', (total_width, total_height), (255, 255, 255))
            ...
            # Add labels
            draw.text((margin, margin - 25), "Original Drawing", fill=(0, 0, 0), font=title_font)
            draw.text((img_width + margin * 2, margin - 25), "Saliency Heatmap", fill=(0, 0, 0), font=title_font)
            draw.text((margin, img_height + margin * 2 - 25), "Important Regions", fill=(0, 0, 0), font=title_font)
            
            # Add explanation text
            text_y = img_height * 2 + margin * 3
            
            # Title
            draw.text((margin, text_y), "Analysis Summary", fill=(0, 0, 0), font=title_font)
            text_y += 30
            ...
            draw.text((margin, text_y), f"Severity: {severity.upper()}", fill=(0, 0, 0), font=text_font)
            text_y += 18
            draw.text((margin, text_y), f"Anomaly Score: {anomaly_score:.3f}", fill=(0, 0, 0), font=text_font)
            text_y += 18
            draw.text((margin, text_y), f"Normalized Score: {normalized_score:.3f}", fill=(0, 0, 0), font=text_font)
            ...
            draw.text((text_x, text_y), f"Important Regions: {num_regions}", fill=(0, 0, 0), font=text_font)
            text_y += 18
            
            for i, bbox in enumerate(bounding_boxes[:3]):  # Show top 3 regions
                importance = bbox.get('importance_score', 0) * 100
                draw.text((text_x, text_y), f"Region {i+1}: {importance:.1f}% importance", 
    ...
    def create_interactive_html_report(self, 
                                     original_image: Union[Image.Image, np.ndarray],
                                     saliency_map: np.ndarray,
                                     explanation: Dict,
                                     base_filename: str) -> str:
