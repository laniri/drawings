...
class AgeGroupManager:
    ...
    def analyze_age_distribution(self, db: Session) -> Dict:
        ...
        ).group_by(func.floor(Drawing.age_years)).order_by('age_floor').all()
    ...
    def suggest_age_groups(self, db: Session) -> List[Tuple[float, float]]:
        ...
        while i < len(age_groups):
            ...
            while (j < len(age_groups) and 
                   total_samples < self.config.min_samples_per_group and
                   (merge_end - group_start) < self.config.max_age_span):
            ...
            if total_samples >= self.config.min_samples_per_group:
                ...
                logger.warning(f"Age group {group_start}-{merge_end} has insufficient data "
    ...
    def create_age_groups(self, db: Session, force_recreate: bool = False) -> List[Dict]:
        """
        ...
        if existing_models and not force_recreate:
            ...
            return [self.model_manager.get_model_info(model.id, db) for model in existing_models]
        ...
        if not suggested_groups:
            raise InsufficientDataError("No viable age groups found with sufficient data")
        ...
        for age_min, age_max in suggested_groups:
            try:
                ...
                logger.info(f"Successfully created model {result['model_id']} "
                          f"for age group {age_min}-{age_max}")
                
            except Exception as e:
                logger.error(f"Failed to create model for age group {age_min}-{age_max}: {str(e)}")
    ...
    def _get_age_group_models(self, db: Session) -> List[AgeGroupModel]:
        ...
        return db.query(AgeGroupModel).filter(
            AgeGroupModel.is_active == True
        ).all()
    
    def find_appropriate_model(self, age: float, db: Session) -> Optional[AgeGroupModel]:
        """
        ...
        models = [model for model in all_models 
                 if model.age_min <= age <= model.age_max]
        ...
        if closest_model and min_distance <= 0.0:  # Exact match only for strict boundary testing
            logger.warning(f"Using closest model for age {age}: "
    ...
    def get_age_group_coverage(self, db: Session) -> Dict:
        ...
        models = db.query(AgeGroupModel).filter(
            AgeGroupModel.is_active == True
        ).order_by(AgeGroupModel.age_min).all()
