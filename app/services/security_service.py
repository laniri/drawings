...
class SecurityService:
    ...
    def validate_iam_least_privilege(self, role_arn: str) -> SecurityValidationResult:
        ...
        try:
            ...
            details['attached_policies'] = [p['PolicyName'] for p in attached_policies['AttachedPolicies']]
        ...
        try:
            ...
            try:
                ...
                if e.response['Error']['Code'] == 'ServerSideEncryptionConfigurationNotFoundError':
                    violations.append(f"Bucket encryption not configured: {bucket_name}")
            ...
            try:
                ...
                if e.response['Error']['Code'] == 'NoSuchPublicAccessBlockConfiguration':
                    violations.append(f"Public access block not configured: {bucket_name}")
                else:
                    raise
    ...
    def validate_security_groups(self, security_group_ids: List[str]) -> SecurityValidationResult:
        """
        ...
        if not self._ec2_client:
            warnings.append("EC2 client not available - skipping security group validation")
            return SecurityValidationResult(
                is_compliant=True,
                violations=violations,
                warnings=warnings,
    ...
    def validate_vpc_configuration(self, vpc_id: str) -> SecurityValidationResult:
        ...
        try:
            ...
            if len(azs) < 2:
                recommendations.append("Use multiple availability zones for high availability")
            ...
            if len(private_subnets) > 0 and len(active_nat_gateways) == 0:
                warnings.append("Private subnets without NAT gateway may lack internet access")
    ...
    def get_current_identity(self) -> Dict[str, Any]:
        ...
        if not self._sts_client:
            return {
                ...
                "reason": "STS client not available"
            }
    ...
    def comprehensive_security_audit(
        ...
        if security_group_ids:
            results['security_groups'] = self.validate_security_groups(security_group_ids)
