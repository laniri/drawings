    ...
    def _validate_config(self):
        ...
        if not (0.0 < self.optimizer.learning_rate <= 1.0):
            raise ValidationError(
                f"Learning rate must be in (0, 1], got {self.optimizer.learning_rate}"
            )

        if not (1 <= self.data.batch_size <= 1024):
            raise ValidationError(
                f"Batch size must be in [1, 1024], got {self.data.batch_size}"
            )
        ...
        if not (0.0 <= self.model.dropout_rate < 1.0):
            raise ValidationError(
                f"Dropout rate must be in [0, 1), got {self.model.dropout_rate}"
            )
...
class ParameterSweepConfig:
    ...
    hidden_dims: List[List[int]] = field(
        default_factory=lambda: [[256, 128, 64], [512, 256, 128], [128, 64, 32]]
    )
...
class TrainingConfigManager:
    ...
    def __init__(self, config_dir: Optional[Path] = None):
        ...
        logger.info(
            f"TrainingConfigManager initialized with config_dir: {self.config_dir}"
        )
    ...
    def create_default_configs(self):
        ...
        self._save_yaml_config(
            self.config_dir / "model" / "autoencoder.yaml", autoencoder_config
        )
    ...
    def create_parameter_sweep(
        self, base_config: TrainingConfig, sweep_config: ParameterSweepConfig
    ) -> List[TrainingConfig]:
        """
        ...
        if len(param_combinations) > sweep_config.max_trials:
            ...
            param_combinations = random.sample(
                param_combinations, sweep_config.max_trials
            )

        # Create configurations
        sweep_configs = []
        for i, (lr, batch_size, hidden_dims, latent_dim, dropout_rate) in enumerate(
            param_combinations
        ):
