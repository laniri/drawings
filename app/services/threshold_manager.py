...
class ThresholdConfig:
    ...
    min_samples_for_calculation: int = (
        30  # Minimum samples needed for reliable threshold
    )
...
class ThresholdManager:
    ...
    def __init__(self, config: ThresholdConfig = None):
        ...
        self._current_percentile = (
            self.config.default_percentile
        )  # Store current percentile

    def calculate_percentile_threshold(
        self, scores: np.ndarray, percentile: float
    ) -> float:
        """
        ...
        if len(scores) == 0:
            raise ThresholdCalculationError(
                "Cannot calculate threshold from empty scores array"
            )
        ...
        if len(valid_scores) == 0:
            raise ThresholdCalculationError(
                "No valid (finite) scores found for threshold calculation"
            )

        if len(valid_scores) < self.config.min_samples_for_calculation:
            logger.warning(
                f"Only {len(valid_scores)} valid scores available for threshold calculation "
        ...
        if not np.isfinite(threshold):
            raise ThresholdCalculationError(
                f"Calculated threshold is not finite: {threshold}"
            )

        logger.info(
            f"Calculated {percentile}th percentile threshold: {threshold:.6f} "
            f"from {len(valid_scores)} scores"
        )

        return float(threshold)

    def calculate_model_threshold(
        self, age_group_model_id: int, db: Session, percentile: Optional[float] = None
    ) -> Dict:
        """
        ...
        try:
            ...
            logger.info(
                f"Calculated thresholds for model {age_group_model_id}: "
            ...
            logger.error(
                f"Failed to calculate threshold for model {age_group_model_id}: {str(e)}"
            )
            raise ThresholdCalculationError(f"Threshold calculation failed: {str(e)}")

    def update_model_threshold(
        self, age_group_model_id: int, new_threshold: float, db: Session
    ) -> bool:
        """
        ...
        try:
            ...
            if not np.isfinite(new_threshold):
                raise ThresholdCalculationError(
                    f"Threshold must be finite, got {new_threshold}"
                )

            if new_threshold < 0:
                raise ThresholdCalculationError(
                    f"Threshold must be non-negative, got {new_threshold}"
                )
            ...
            keys_to_remove = [
                key
                for key in self._cached_thresholds.keys()
            ...
            logger.info(
                f"Updated threshold for model {age_group_model_id}: "
            ...
            logger.error(
                f"Failed to update threshold for model {age_group_model_id}: {str(e)}"
            )
            db.rollback()
            raise ThresholdCalculationError(f"Threshold update failed: {str(e)}")

    def recalculate_all_thresholds(
        self, db: Session, percentile: Optional[float] = None
    ) -> Dict:
        """
        ...
        for model in models:
            try:
                ...
                success = self.update_model_threshold(
                    model.id, threshold_info["primary_threshold"], db
                )
                ...
                logger.error(
                    f"Failed to recalculate threshold for model {model.id}: {str(e)}"
                )
        ...
        logger.info(
            f"Threshold recalculation complete: "
    ...
    def get_threshold_for_age(self, age: float, db: Session) -> Optional[float]:
        """
    ...
    def is_anomaly(
        self, anomaly_score: float, age: float, db: Session
