...
class DrawingMetadata(BaseModel):
    ...
    expert_label: Optional[str] = Field(None, max_length=50, description="Expert annotation")
    drawing_tool: Optional[str] = Field(None, max_length=50, description="Drawing tool used")
    prompt: Optional[str] = Field(None, max_length=500, description="Drawing prompt given")
...
class DataPipelineService:
    ...
    def validate_image(self, image_data: bytes, filename: Optional[str] = None) -> ValidationResult:
        """
    ...
    def preprocess_image(self, image_data: bytes, target_size: Optional[Tuple[int, int]] = None) -> np.ndarray:
        """
        ...
        try:
            ...
            if not validation_result.is_valid:
                raise ImageProcessingError(f"Image validation failed: {validation_result.error_message}")
            ...
            if image_array.shape[-1] != 3:
                raise ImageProcessingError(f"Unexpected number of channels: {image_array.shape[-1]}")
            
            logger.debug(f"Image preprocessed successfully: shape={image_array.shape}, dtype={image_array.dtype}")
            return image_array
    ...
    def validate_and_preprocess(self, image_data: bytes, metadata: Dict[str, Any]) -> Tuple[np.ndarray, DrawingMetadata]:
        """
