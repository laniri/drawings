...
from app.models.database import Drawing, AgeGroupModel
...
class DataSufficiencyWarning:
    ...
    warning_type: str  # "insufficient_data", "unbalanced_distribution", "narrow_age_range"
...
class DataSufficiencyAnalyzer:
    ...
    def analyze_age_group_data(self, 
                             age_min: float, 
                             age_max: float, 
                             db: Session) -> AgeGroupDataInfo:
        """
    ...
    def _calculate_data_quality_score(self, drawings: List[Any]) -> float:
        ...
        age_distribution_score = min(age_range / 1.0, 1.0)  # Good if spans at least 1 year
        ...
        subject_diversity_score = min(unique_subjects / 3.0, 1.0)  # Good if at least 3 subjects
        ...
        metadata_completeness_score = complete_metadata_count / len(drawings) if drawings else 0
    ...
    def generate_data_warnings(self, 
                             age_groups: List[Tuple[float, float]], 
                             db: Session) -> List[DataSufficiencyWarning]:
        """
        ...
        try:
            ...
            for age_min, age_max in age_groups:
                ...
                if data_info.age_distribution:
                    age_span = max(data_info.age_distribution) - min(data_info.age_distribution)
    ...
    def suggest_age_group_merging(self, 
                                age_groups: List[Tuple[float, float]], 
                                db: Session) -> List[AgeGroupMergingSuggestion]:
        """
        ...
        try:
            ...
            insufficient_groups = [
                (age_min, age_max, info) for age_min, age_max, info in group_data
            ...
            for i, (age_min1, age_max1, info1) in enumerate(insufficient_groups):
                ...
                for age_min2, age_max2, info2 in group_data:
                    ...
                    if self._are_groups_mergeable(age_min1, age_max1, age_min2, age_max2):
                        ...
                        if merged_span <= self.max_age_group_span:
                            ...
                            suggestions.append(AgeGroupMergingSuggestion(
                                original_groups=[(age_min1, age_max1), (age_min2, age_max2)],
            ...
            logger.info(f"Generated {len(unique_suggestions)} age group merging suggestions")
    ...
    def _are_groups_mergeable(self, 
                            age_min1: float, age_max1: float,
                            age_min2: float, age_max2: float) -> bool:
        """Check if two age groups can be merged."""
        # Groups are mergeable if they are adjacent or overlapping
        return (age_max1 >= age_min2 and age_min1 <= age_max2) or \
               (abs(age_max1 - age_min2) <= 0.5) or \
               (abs(age_max2 - age_min1) <= 0.5)
    
    def _calculate_improvement_score(self, 
                                   samples1: int, 
                                   samples2: int, 
                                   combined_samples: int) -> float:
        """Calculate improvement score for merging (0.0 to 1.0)."""
        # Base score: how much the combined group improves sufficiency
        sufficiency_before = min(samples1 / self.min_samples_per_group, 1.0) + \
                           min(samples2 / self.min_samples_per_group, 1.0)
    ...
    def _generate_merging_rationale(self, 
                                  group1: Tuple[float, float, int],
                                  group2: Tuple[float, float, int],
                                  combined_samples: int) -> str:
        ...
        rationale = f"Merging age groups {age_min1}-{age_max1} ({samples1} samples) and " \
        ...
        if combined_samples >= self.recommended_samples_per_group:
            ...
            rationale += " This improves data sufficiency but may still require additional data."
...
class DataAugmentationSuggester:
    ...
    def suggest_augmentation_strategies(self, 
                                     age_group_data: AgeGroupDataInfo) -> Dict[str, Any]:
        """
        ...
        if age_group_data.sample_count < 20:
            ...
            suggestions["recommended_techniques"].extend(self.augmentation_techniques["geometric"])
            suggestions["recommended_techniques"].extend(self.augmentation_techniques["appearance"])
            ...
            suggestions["recommended_techniques"].extend(["Rotation (±10 degrees)", "Translation (±5%)"])
