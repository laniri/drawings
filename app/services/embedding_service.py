...
class SubjectEncoder:
    ...
    @classmethod
    def encode_subject_category(cls, subject: Optional[Union[str, SubjectCategory]]) -> np.ndarray:
        """
        ...
        try:
            ...
            if isinstance(subject, str):
                try:
                    ...
                    for category in SubjectCategory:
                        ...
                        logger.warning(f"Unknown subject category '{subject}', using 'unspecified'")
                        subject = SubjectCategory.UNSPECIFIED
                except Exception:
                    # If any error occurs, use unspecified
                    logger.warning(f"Error processing subject category '{subject}', using 'unspecified'")
                    subject = SubjectCategory.UNSPECIFIED
            
            # At this point, subject should be a SubjectCategory enum
            if not isinstance(subject, SubjectCategory):
                logger.warning(f"Invalid subject type '{type(subject)}', using 'unspecified'")
                subject = SubjectCategory.UNSPECIFIED
            
            # Get the index for this subject
            if subject not in cls.SUBJECT_TO_INDEX:
                raise SubjectEncodingError(f"Subject category '{subject}' not found in mapping")
            ...
            if isinstance(e, SubjectEncodingError):
                raise
            raise SubjectEncodingError(f"Failed to encode subject category '{subject}': {str(e)}")
    ...
    def decode_subject_encoding(cls, encoding: np.ndarray) -> SubjectCategory:
        ...
        try:
            ...
            if subject_index not in cls.INDEX_TO_SUBJECT:
                raise SubjectEncodingError(f"Subject index {subject_index} not found in mapping")
...
class EmbeddingService:
    ...
    def get_service_info(self) -> Dict:
        ...
        if self.is_ready():
            info.update({
                "hybrid_embedding_dimension": self.get_embedding_dimension(hybrid=True),
                ...
                "supported_subject_categories": len(SubjectEncoder.get_supported_categories()),
                "subject_categories": [cat.value for cat in SubjectEncoder.get_supported_categories()]
            })
    ...
    def _generate_cache_key(self, image_tensor: torch.Tensor, age: Optional[float] = None, subject: Optional[Union[str, SubjectCategory]] = None) -> str:
    ...
    def generate_hybrid_embedding(self,
        ...
        try:
            ...
            raise EmbeddingGenerationError(f"Failed to generate hybrid embedding: {str(e)}")
    
    def _generate_visual_embedding(self,
                                 image: Union[Image.Image, np.ndarray],
                                 use_cache: bool = True) -> np.ndarray:
        """
        ...
        try:
            ...
            raise EmbeddingGenerationError(f"Failed to generate visual embedding: {str(e)}")
    
    def separate_embedding_components(self, hybrid_embedding: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        try:
            ...
            if isinstance(e, EmbeddingGenerationError):
                raise
            raise EmbeddingGenerationError(f"Failed to separate embedding components: {str(e)}")
    ...
    def generate_embedding(self, 
        ...
        Returns:
            numpy array containing the embedding vector
        """
        logger.warning("generate_embedding() is deprecated. Use generate_hybrid_embedding() instead.")
    ...
    def batch_embed(self,
        ...
        if subjects is not None and len(subjects) != len(images):
            raise EmbeddingGenerationError("Number of subjects must match number of images")
        ...
        try:
            for i in range(0, len(images), batch_size):
                ...
                batch_subjects = subjects[i:i + batch_size] if subjects else [None] * len(batch_images)
                batch_ages = ages[i:i + batch_size] if ages else [None] * len(batch_images)
                
                # Process each image in the batch
                batch_embeddings = []
                for image, subject, age in zip(batch_images, batch_subjects, batch_ages):
                    embedding = self.generate_hybrid_embedding(image, subject, age, use_cache)
            ...
            raise EmbeddingGenerationError(f"Failed to generate batch hybrid embeddings: {str(e)}")
    ...
    def generate_batch_embeddings(self, 
        ...
        """
        logger.warning("generate_batch_embeddings() is deprecated. Use batch_embed() instead.")
    ...
    def get_embedding_dimension(self, include_age: bool = False, hybrid: bool = True) -> int:
        """
    ...
    def store_hybrid_embedding_for_db(self,
                                     drawing_id: int,
                                     hybrid_embedding: np.ndarray,
                                     subject: Optional[Union[str, SubjectCategory]] = None,
                                     age: Optional[float] = None,
                                     model_type: str = "vit") -> Tuple[bytes, int, bytes, bytes]:
        """
        ...
        try:
            ...
            # Separate components
            visual_component, subject_component = self.separate_embedding_components(hybrid_embedding)
            ...
            subject_bytes = self.embedding_storage.serialize_embedding(subject_component)
            
            return hybrid_bytes, 832, visual_bytes, subject_bytes
            
        except Exception as e:
            raise EmbeddingGenerationError(f"Failed to store hybrid embedding: {str(e)}")
    ...
    def retrieve_hybrid_embedding_from_db(self,
                                         drawing_id: int,
        ...
        try:
            ...
            if serialized_data is not None:
                hybrid_embedding = self.embedding_storage.deserialize_embedding(serialized_data)
            ...
            if visual_component_data is not None and subject_component_data is not None:
                visual_component = self.embedding_storage.deserialize_embedding(visual_component_data)
                subject_component = self.embedding_storage.deserialize_embedding(subject_component_data)
                
                if (visual_component is not None and visual_component.shape == (768,) and
                    subject_component is not None and subject_component.shape == (64,)):
                    # Reconstruct hybrid embedding
                    hybrid_embedding = np.concatenate([visual_component, subject_component], axis=0)
                    return hybrid_embedding
            
            return None
            
        except Exception as e:
            logger.error(f"Failed to retrieve hybrid embedding for drawing {drawing_id}: {str(e)}")
            return None
