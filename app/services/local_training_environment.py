...
class TrainingProgress:
    ...
    def batch_progress(self) -> float:
        ...
        return (
            (self.batch / self.total_batches) * 100 if self.total_batches > 0 else 0.0
        )
...
class DeviceManager:
    ...
    def _setup_cuda_device(self) -> None:
        ...
        logger.info(
            f"CUDA device configured: {props.name} with {self._device_info['total_memory_gb']:.1f}GB memory"
        )
    ...
    def _setup_cpu_device(self) -> None:
        ...
        logger.info(
            f"CPU device configured: {cpu_count}/{cpu_count_logical} cores, "
...
class TrainingProgressMonitor:
    ...
    def start_epoch(self, epoch: int, total_batches: int) -> None:
        ...
        logger.info(
            f"Starting epoch {epoch}/{self.total_epochs} with {total_batches} batches"
        )

    def update_batch(
        self, batch: int, train_loss: float, learning_rate: float = 0.0
    ) -> None:
        ...
        if batch > 0:
            ...
            if epochs_remaining > 0:
                avg_epoch_time = (
                    elapsed_time / self.current_epoch
    ...
    def update_epoch(self, epoch: int, train_loss: float, val_loss: float) -> None:
        ...
        logger.info(
            f"Epoch {epoch}/{self.total_epochs} completed: "
...
class LocalTrainingEnvironment:
    ...
    def prepare_training_data(
        self, dataset_folder: str, metadata_file: str, config: TrainingConfig
        ...
        try:
            ...
            logger.info(
                f"Dataset split: train={dataset_split.train_count}, "
            ...
            test_embeddings = (
                self._generate_embeddings_for_split(
                    dataset_split.test_files, dataset_split.test_metadata
                )
                if dataset_split.test_count > 0
                else np.array([])
            )

            logger.info(
                f"Generated embeddings: train={len(train_embeddings)}, "
    ...
    def _generate_embeddings_for_split(
        self, files: List[Path], metadata: List
    ) -> np.ndarray:
        ...
        for file_path, meta in zip(files, metadata):
            try:
                ...
                embedding = self.embedding_service.generate_embedding(
                    image=image, age=meta.age_years, use_cache=True
                )
        ...
        if progress.batch % 10 == 0:  # Log every 10 batches
            ...
            if progress.memory_usage:
                if "allocated_mb" in progress.memory_usage:
                    memory_str = (
                        f", GPU Memory: {progress.memory_usage['allocated_mb']:.0f}MB"
                    )
                elif "used_mb" in progress.memory_usage:
                    memory_str = f", Memory: {progress.memory_usage['percent']:.1f}%"

            logger.info(
                f"Job {progress.job_id} - Epoch {progress.epoch}/{progress.total_epochs}, "
                f"Batch {progress.batch}/{progress.total_batches}, "
                f"Loss: {progress.train_loss:.6f}{memory_str}"
            )

    def _save_training_results(
        self, job_id: int, training_result: Dict, config: TrainingConfig, db: Session
    ) -> None:
        ...
        try:
            ...
            training_report = TrainingReport(
                ...
                - training_result["best_val_loss"],  # Approximate
        ...
        try:
            ...
            if metrics:
                ...
                if arch_info:
                    ...
                    info_text += (
                        f"Parameters: {arch_info.get('total_parameters', 'N/A')}"
                    )
                    axes[1, 1].text(
                        0.1,
                        0.5,
                ...
                plt.tight_layout()
                plt.savefig(
                    output_dir / "training_metrics.png", dpi=300, bbox_inches="tight"
                )
                plt.close()
    ...
    def list_training_jobs(self, db: Session) -> List[Dict]:
        ...
        jobs = (
            ...
            .filter(TrainingJob.environment == "local")
...
class EnhancedAutoencoderTrainer(AutoencoderTrainer):
    ...
    def train(self, train_embeddings: np.ndarray, val_embeddings: np.ndarray) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Enhanced training completed in {training_time:.1f}s. "
    ...
    def _create_data_loader(
        self, embeddings: np.ndarray, shuffle: bool = True
    ) -> DataLoader:
    ...
    def _train_epoch(self, train_loader: DataLoader, epoch: int) -> float:
        ...
        for batch_idx, (batch_data, batch_target) in enumerate(train_loader):
            ...
            self.progress_monitor.update_batch(batch_idx + 1, loss.item(), current_lr)
    ...
    def _calculate_enhanced_metrics(
        self, train_embeddings: np.ndarray, val_embeddings: np.ndarray
    ) -> Dict:
        ...
        return {
            **train_metrics,
            **val_metrics,
            "generalization_gap": val_metrics["validation_mean_error"]
            - train_metrics["train_mean_error"],
        }
    ...
    def _calculate_set_metrics(self, embeddings: np.ndarray, prefix: str) -> Dict:
        ...
        with torch.no_grad():
            ...
            for p in [90, 95, 99]:
                idx = int(p / 100.0 * n)
                percentiles[f"{prefix}_percentile_{p}"] = sorted_errors[
                    min(idx, n - 1)
                ].item()
