...
class MonitoringService:
    ...
    def __init__(
        ...
        self.performance_thresholds = (
            performance_thresholds or settings.PERFORMANCE_THRESHOLDS
        )
        ...
        try:
            ...
            if os.getenv("ENVIRONMENT") == "production":
                ...
                try:
                    ...
                    should_initialize = getattr(
                        settings, "is_production", False
                    ) and getattr(settings, "MONITORING_ENABLED", True)
                except Exception:
    ...
    def log_error(
        ...
        try:
            ...
            self.record_performance_metrics(
                {"error_count": 1, "log_entry_count": 1}, correlation_id=correlation_id
            )
    ...
    def log_structured(
        ...
        try:
            ...
            monitoring_logger = logging.getLogger("monitoring")
            log_method = getattr(
                monitoring_logger, level.lower(), monitoring_logger.info
            )
    ...
    def should_send_alert(self, error_type: str, correlation_id: str) -> bool:
        ...
        same_type_alerts = [
            alert for alert in recent_alerts if error_type in str(alert.correlation_id)
        ]
    ...
    def record_performance_metrics(
        self, metrics: Dict[str, float], correlation_id: Optional[str] = None
    ) -> MetricResult:
        """
                ...
                cloudwatch_metrics.append(
                    {
                        ...
                        "Timestamp": timestamp,
                        "Dimensions": [
                            {"Name": "Service", "Value": "drawing-analysis"},
                            {
                                "Name": "Environment",
                                "Value": (
                                    "production"
    ...
    def send_performance_alert(
        ...
        return self.send_alert(
            level=level, message=message, correlation_id=correlation_id, details=details
        )
    ...
    def setup_cost_monitoring(self) -> bool:
        ...
        try:
            ...
            topic_arn = getattr(settings, "SNS_ALERT_TOPIC_ARN", None) or os.getenv(
                "SNS_ALERT_TOPIC_ARN"
            )
    ...
    def _send_to_cloudwatch_logs(self, log_data: Dict[str, Any]):
        ...
        try:
            ...
            log_group_name = os.getenv(
                "CLOUDWATCH_LOG_GROUP", "/aws/ecs/drawing-analysis"
            )
            try:
                from app.core.config import settings

                log_group_name = getattr(
                    settings, "CLOUDWATCH_LOG_GROUP", log_group_name
                )
            except Exception:
                pass
