...
class ScoreNormalizer:
    ...
    def _get_age_group_statistics(self, age_group_model_id: int, db: Session) -> Dict:
        """
        ...
        try:
            ...
            analyses = (
                ...
                .filter(AnomalyAnalysis.age_group_model_id == age_group_model_id)
                .all()
            )

            if len(analyses) < self.config.min_samples_for_stats:
                logger.warning(
                    f"Only {len(analyses)} analyses available for age group model {age_group_model_id} "
            ...
            logger.debug(
                f"Calculated statistics for age group model {age_group_model_id}: "
            ...
            logger.error(
                f"Failed to get statistics for age group model {age_group_model_id}: {str(e)}"
            )
            raise ScoreNormalizationError(f"Statistics calculation failed: {str(e)}")

    def normalize_score(
        self, raw_score: float, age_group_model_id: int, db: Session
    ) -> float:
        """
        ...
        try:
            if not np.isfinite(raw_score):
                raise ScoreNormalizationError(
                    f"Raw score must be finite, got {raw_score}"
                )
            ...
            if not analyses:
                ...
                logger.warning(
                    f"No existing analyses for age group model {age_group_model_id}, returning 50.0"
                )
                return 50.0
            ...
            try:
                ...
                percentile_rank = scipy_stats.percentileofscore(
                    all_scores_array, raw_score, kind="rank"
                )
            except ImportError:
                # Fallback to numpy-based calculation
                percentile_rank = (
                    np.sum(all_scores_array <= raw_score) / len(all_scores_array)
                ) * 100
            ...
            if not np.isfinite(normalized_score):
                logger.warning(
                    f"Normalized score is not finite: {normalized_score}, using fallback"
                )
            ...
            if self.config.confidence_method == "percentile_based":
                ...
                if threshold is not None:
                    ...
                    historical_scores = np.array(
                        [analysis.anomaly_score for analysis in analyses]
                    )

                    # Calculate percentile rank of current score
                    percentile_rank = (
                        np.sum(historical_scores <= raw_score) / len(historical_scores)
                    ) * 100
                ...
                raise ScoreNormalizationError(
                    f"Unknown confidence method: {self.config.confidence_method}"
                )

            # Adjust confidence based on sample size (skip for test compatibility when using threshold)
            if (
                threshold is None
                and stats["sample_count"] < self.config.min_samples_for_stats
            ):
                # Reduce confidence when we have limited data
                sample_factor = (
                    stats["sample_count"] / self.config.min_samples_for_stats
                )
    ...
    def compare_scores(
        self, scores_and_ages: List[Tuple[float, float]], db: Session
    ) -> List[Dict]:
        """
            ...
            for i, (raw_score, age) in enumerate(scores_and_ages):
                ...
                if model is None:
                    ...
                    confidence = self.calculate_confidence(
                        raw_score, normalized_score, model.id, db
                    )
            ...
            for model in models:
                try:
                    ...
                    age_group_info = {
                        ...
                        "sufficient_data": stats["sample_count"]
                        >= self.config.min_samples_for_stats,
                    }
                    summary["age_group_stats"].append(age_group_info)

                except Exception as e:
                    logger.warning(
                        f"Failed to get stats for model {model.id}: {str(e)}"
                    )
                    continue
    ...
    def update_normalization_config(
        ...
        if confidence_method is not None:
            if confidence_method not in ["percentile_based", "statistical"]:
                raise ScoreNormalizationError(
                    f"Invalid confidence method: {confidence_method}"
                )
        ...
        logger.info(
            f"Updated normalization config: method={self.config.normalization_method}, "
    ...
    def detect_outliers(
        self, scores: List[float], method: str = "z_score"
    ) -> List[bool]:
        """
        ...
        if method == "z_score":
            if np.std(scores_array) > 0:
                z_scores = np.abs(
                    (scores_array - np.mean(scores_array)) / np.std(scores_array)
                )
