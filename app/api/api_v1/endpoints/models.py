...
from app.services.data_sufficiency_service import (
    ...
    get_data_augmentation_suggester,
    get_data_sufficiency_analyzer,
)
from app.services.model_manager import TrainingConfig, get_model_manager
...
async def train_age_group_model(
    ...
    try:
        ...
        background_tasks.add_task(train_model_background, job_id, request, db)
...
async def train_model_background(
    job_id: str, request: ModelTrainingRequest, db: Session
):
    ...
    try:
        ...
        training_tracker.update_training(
            job_id, status="training", progress=10, message="Preparing training data"
        )
        ...
        # Update progress
        training_tracker.update_training(
            job_id, progress=20, message="Starting model training"
        )

        # Train the model
        result = model_manager.train_age_group_model(
            age_min=request.age_min, age_max=request.age_max, config=config, db=db
        )
        ...
        logger.info(
            f"Training job {job_id} completed successfully, model ID: {result['model_id']}"
        )
...
async def update_model_threshold(
    model_id: int, request: ThresholdUpdateRequest, db: Session = Depends(get_db)
):
    """
...
async def get_model_status(db: Session = Depends(get_db)):
    ...
    try:
        ...
        last_model = (
            db.query(AgeGroupModel)
            .order_by(AgeGroupModel.created_timestamp.desc())
            .first()
        )
...
async def auto_create_age_groups(
    ...
    try:
        ...
        background_tasks.add_task(
            auto_create_models_background, job_id, force_recreate, db
        )
...
async def auto_create_models_background(job_id: str, force_recreate: bool, db: Session):
    ...
    try:
        ...
        logger.info(
            f"Auto-creation job {job_id} completed: {len(created_models)} models created"
        )
...
async def analyze_data_sufficiency(
    age_groups: Optional[str] = None, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        if age_groups:
            ...
            if not age_group_list:
                age_group_list = [
                    (i, i + 1) for i in range(3, 12)
                ]  # 3-4, 4-5, ..., 11-12

        # Analyze each age group
        analysis_results = []
        for age_min, age_max in age_group_list:
            data_info = data_sufficiency_analyzer.analyze_age_group_data(
                age_min, age_max, db
            )
        ...
        merging_suggestions = data_sufficiency_analyzer.suggest_age_group_merging(
            age_group_list, db
        )
...
async def analyze_specific_age_group(
    age_min: float, age_max: float, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        augmentation_suggestions = (
            data_augmentation_suggester.suggest_augmentation_strategies(data_info)
        )

        return {
            "success": True,
            "age_group": {"min": age_min, "max": age_max},
            ...
            "recommendations": {
                ...
                "quality_rating": (
                    "excellent"
                    if data_info.data_quality_score > 0.8
                    else (
                        "good"
...
async def merge_age_groups(
    ...
    try:
        ...
        combined_count = (
            db.query(Drawing)
            .filter(Drawing.age_years >= merged_min, Drawing.age_years < merged_max)
            .count()
        )
        ...
        merged_model = AgeGroupModel(
            ...
            parameters=json.dumps(
                {"merged_from": original_groups, "created_by_merge": True}
            ),
        ...
        logger.info(
            f"Merged age groups {original_groups} into {merged_min}-{merged_max}"
        )
