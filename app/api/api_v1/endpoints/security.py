...
from app.services.security_service import get_security_service, SecurityValidationResult
...
class SecurityAuditRequest(BaseModel):
    ...
    s3_buckets: Optional[List[str]] = Field(None, description="S3 bucket names to validate")
    security_group_ids: Optional[List[str]] = Field(None, description="Security group IDs to validate")
...
async def validate_security_groups(
    security_group_ids: List[str] = Query(..., description="Security group IDs to validate"),
    security_service = Depends(get_security_service_dependency)
):
    """
...
@router.get("/validate/encryption-in-transit", response_model=SecurityValidationResponse)
...
async def comprehensive_security_audit(
    ...
    try:
            ...
            if isinstance(result, SecurityValidationResult):
                ...
                if component_name == 's3_buckets':
                    for bucket_name, bucket_result in result.items():
                        components[f"s3_bucket_{bucket_name}"] = SecurityValidationResponse(
        ...
        summary = {
            ...
            "compliant_components": sum(1 for comp in components.values() if comp.is_compliant),
            ...
            "recommendations_count": sum(len(comp.recommendations) for comp in components.values())
        }
...
async def get_compliance_report(
    ...
    try:
        ...
        if format.lower() == "csv":
            ...
            compliance_data["csv_download_url"] = "/api/v1/security/compliance/download?format=csv"
