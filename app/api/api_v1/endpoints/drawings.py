...
@router.post(
    "/upload", response_model=DrawingResponse, status_code=status.HTTP_201_CREATED
)
async def upload_drawing(
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Drawing uploaded successfully: ID={drawing.id}, filename={filename}"
            )

            # Schedule background preprocessing (for future embedding generation)
            background_tasks.add_task(
                preprocess_drawing_background, drawing.id, file_path
            )
...
async def preprocess_drawing_background(drawing_id: int, file_path: str):
    ...
    try:
        ...
        logger.info(
            f"Background preprocessing queued for drawing {drawing_id} at {file_path}"
        )
    except Exception as e:
        logger.error(
            f"Background preprocessing failed for drawing {drawing_id}: {str(e)}"
        )
...
async def list_drawings(
    ...
    drawings = (
        query.order_by(Drawing.upload_timestamp.desc())
        .offset(offset)
        .limit(page_size)
        .all()
    )
    ...
    try:
        ...
        for i, file in enumerate(files):
            try:
                ...
                file_content = await file.read()
                validation_result = data_pipeline.validate_image(
        ...
        upload_progress.update(
            upload_id, 100.0, f"completed_{successful}_successful_{len(errors)}_errors"
        )

        logger.info(
            f"Batch upload {upload_id} completed: {successful}/{total_files} successful"
        )
...
async def get_drawing_stats(db: Session = Depends(get_db)):
    ...
    try:
        ...
        for label in ExpertLabel:
            count = (
                db.query(Drawing).filter(Drawing.expert_label == label.value).count()
            )
