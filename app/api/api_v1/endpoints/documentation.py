...
class DocumentationStatus(BaseModel):
    ...
    is_generating: bool = Field(..., description="Whether documentation is currently being generated")
...
class DocumentationMetrics(BaseModel):
    ...
    average_duration: float = Field(..., description="Average generation duration in seconds")
    success_rate: float = Field(..., description="Success rate percentage")
    file_breakdown: Dict[str, int] = Field(default_factory=dict, description="Files by category")
    validation_status: Dict[str, Any] = Field(default_factory=dict, description="Validation results")


class GenerationRequest(BaseModel):
    """Documentation generation request model."""
    categories: Optional[List[str]] = Field(None, description="Specific categories to generate")
    force: bool = Field(False, description="Force regeneration even if no changes detected")
...
class NavigationContext(BaseModel):
    ...
    cross_references: List[Dict[str, Any]] = Field(default_factory=list, description="Cross-references")
    related_content: List[Dict[str, Any]] = Field(default_factory=list, description="Related content")
    prev_document: Optional[Dict[str, Any]] = Field(None, description="Previous document")
...
async def get_documentation_metrics():
    ...
    if _generation_history:
        successful = sum(1 for result in _generation_history if result.get("success", False))
        _documentation_metrics["success_rate"] = (successful / len(_generation_history)) * 100
        
        # Calculate average duration
        durations = [result.get("duration", 0) for result in _generation_history if result.get("duration")]
        if durations:
...
async def get_documentation_files(
    ...
    for file_path in docs_dir.rglob("*.md"):
        try:
            ...
            file_category = str(relative_path.parts[0]) if relative_path.parts else "root"
            ...
            if search:
                ...
                if (search_lower not in file_path.name.lower() and 
                    search_lower not in content.lower()):
                    continue
...
async def validate_documentation(
    ...
    if _generation_status["is_generating"]:
        raise HTTPException(
            status_code=409,
            detail="Cannot validate while generation is in progress"
        )
...
async def preview_documentation_changes(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Unknown documentation category: {category}"
            )
...
async def batch_generate_documentation(
    ...
    if not batch_requests:
        raise HTTPException(
            status_code=400,
            detail="No batch requests provided"
        )
    ...
    for i, batch_req in enumerate(batch_requests):
        if "categories" not in batch_req:
            raise HTTPException(
                status_code=400,
                detail=f"Batch request {i} missing 'categories' field"
            )
...
async def get_generation_schedule():
    ...
    if _generation_status.get("start_time"):
        ...
        if _documentation_metrics.get("average_duration"):
            estimated_end = start_time + timedelta(seconds=_documentation_metrics["average_duration"])
...
async def schedule_generation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Invalid schedule_time format. Use ISO format."
        )
    ...
    try:
        ...
        if categories:
            ...
            for i, category in enumerate(categories):
                try:
                    doc_type = DocumentationType(category)
                    _generation_status["current_task"] = f"Generating {category} documentation"
                    _generation_status["progress"] = 30 + (i * 40 // len(categories))
                    _generation_status["last_update"] = datetime.now()
                    
                    category_result = engine.generate_category(doc_type)
                    result.generated_files.extend([str(f) for f in category_result.generated_files])
...
async def _run_batch_generation(batch_requests: List[Dict[str, Any]], schedule_delay: int = 0):
    ...
    for i, batch_req in enumerate(batch_requests):
        try:
            _generation_status["current_task"] = f"Batch {i+1}/{len(batch_requests)}: {batch_req.get('name', 'Unnamed')}"
        ...
        for category_dir in docs_dir.iterdir():
            if category_dir.is_dir() and not category_dir.name.startswith('.'):
                ...
                status["categories"][category_dir.name] = {
                    ...
                    "validated": len(category_files),  # Assume all are validated for now
                    "errors": 0,
                    "warnings": 0
                }
...
async def get_navigation_context(document_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get navigation context: {str(e)}"
        )
...
async def rebuild_navigation_structure(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to start navigation rebuild: {str(e)}"
        )
