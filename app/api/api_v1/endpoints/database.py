...
async def create_database_backup(request: BackupRequest):
    ...
    try:
        if request.include_files:
            ...
            backup_info = (
                await database_migration_service.backup_service.create_full_backup(
                    include_files=True
                )
            )
...
async def get_migration_info():
    ...
    try:
        ...
        return {"status": "success", "migration_info": migration_info}

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get migration info: {str(e)}"
        )
...
async def validate_cross_environment_consistency(request: ConsistencyCheckRequest):
    ...
    try:
        validation_result = (
            await database_migration_service.validate_cross_environment_consistency(
...
async def list_backups():
    ...
    try:
        ...
        return {"status": "success", "backups": backup_list}
...
async def schedule_automated_backups(
    background_tasks: BackgroundTasks, interval_hours: int = 6
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to schedule backups: {str(e)}"
        )
...
async def run_consistency_check():
    ...
    try:
        consistency_result = (
            await database_migration_service._validate_migration_consistency()
        )
