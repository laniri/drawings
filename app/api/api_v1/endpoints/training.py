...
from app.services.sagemaker_training_service import get_sagemaker_training_service, SageMakerError
from app.services.local_training_environment import get_local_training_environment, LocalTrainingError
from app.services.training_config import TrainingConfig, TrainingConfigManager, get_training_config_manager
from app.services.model_deployment_service import (
    get_model_exporter, get_model_validator, get_model_deployment_service,
    ModelDeploymentConfig, ModelExportError, ModelValidationError, ModelDeploymentError
)
...
# Initialize services
sagemaker_service = get_sagemaker_training_service()  # May be None if AWS not configured
...
async def submit_training_job(
    ...
    try:
        ...
        if request.environment == TrainingEnvironment.LOCAL:
            ...
            job_id = await submit_sagemaker_training_job(training_config, background_tasks, db)
...
async def submit_local_training_job(config: TrainingConfig, db: Session) -> int:
    ...
    try:
        ...
        train_embeddings, val_embeddings, test_embeddings = local_training_service.prepare_training_data(
...
async def submit_sagemaker_training_job(
    ...
    try:
        ...
        if not validation_result['is_valid']:
            raise SageMakerError(f"SageMaker configuration invalid: {validation_result['errors']}")
        
        # Schedule SageMaker job preparation in background
        background_tasks.add_task(
            prepare_and_submit_sagemaker_job,
            config,
            db
        )
...
async def prepare_and_submit_sagemaker_job(config: TrainingConfig, db: Session):
    ...
    try:
        ...
        role_arn = sagemaker_service.create_execution_role("SageMakerDrawingAnomalyRole")
...
async def get_training_reports(
    ...
    try:
        ...
        reports = db.query(TrainingReport).filter(
            TrainingReport.training_job_id == job_id
        ).order_by(TrainingReport.created_timestamp.desc()).all()
...
async def deploy_trained_model(
    ...
    try:
        ...
        logger.info(f"Deployed model {new_model.id} for age group {request.age_group_min}-{request.age_group_max}")
    ...
    try:
        ...
        if sagemaker_service is not None:
            ...
            sagemaker_validation = {
                "is_valid": False,
                "errors": ["SageMaker service not available - AWS credentials not configured"]
            }
...
async def setup_sagemaker_environment(
    ...
    try:
        ...
        if ecr_repository:
            ...
            ecr_uri = sagemaker_service.container_builder.push_to_ecr(container_tag, ecr_repository)
...
async def validate_exported_model(
    model_id: str
):
    """
