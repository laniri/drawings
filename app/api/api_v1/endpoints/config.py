...
async def update_config(
    request: ConfigurationUpdateRequest, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        return SuccessResponse(
            message="Configuration updated successfully", data=updates
        )
...
async def update_threshold_settings(
    request: ThresholdUpdateRequest, db: Session = Depends(get_db)
):
    """
...
async def update_age_grouping(
    ...
    try:
        ...
        return SuccessResponse(
            message="Age grouping configuration updated", data=updates
        )
    ...
    try:
        ...
        try:
            ...
            active_models = (
                ...
                .filter(AgeGroupModel.is_active.is_(True))
                .count()
            )
        ...
        if all(
            "error" not in status
            for status in [database_status, models_status, storage_status]
        ):
...
async def get_supported_subject_categories(db: Session = Depends(get_db)):
    ...
    try:
        ...
        usage_stats = (
            db.query(Drawing.subject, func.count(Drawing.id).label("count"))
        ...
        drawings_with_subject = (
            db.query(Drawing).filter(Drawing.subject.isnot(None)).count()
        )
        ...
        return {
            ...
            "statistics": {
                ...
                "subject_coverage_percentage": (
                    round((drawings_with_subject / total_drawings * 100), 1)
                    if total_drawings > 0
                    else 0
                ),
            },
...
async def get_subject_specific_statistics(
    ...
    try:
        ...
        if subject:
            ...
            subject_drawings = (
                db.query(Drawing).filter(Drawing.subject == subject).count()
            )
            ...
            analysis_stats = (
                db.query(
                    ...
                    func.sum(func.cast(AnomalyAnalysis.is_anomaly, func.Integer)).label(
                        "anomaly_count"
                    ),
                    func.avg(AnomalyAnalysis.anomaly_score).label("avg_anomaly_score"),
                    func.avg(AnomalyAnalysis.normalized_score).label(
            ...
            attribution_stats = (
                ...
                .join(Drawing, AnomalyAnalysis.drawing_id == Drawing.id)
                .filter(Drawing.subject == subject)
                .group_by(AnomalyAnalysis.anomaly_attribution)
                .all()
            )
            ...
            return {
                ...
                "analysis_statistics": {
                    ...
                    "normal_count": (analysis_stats.total_analyses or 0)
                    - (analysis_stats.anomaly_count or 0),
                    "anomaly_rate": round(
                        (analysis_stats.anomaly_count or 0)
                        / max(1, analysis_stats.total_analyses or 1)
                        * 100,
                        1,
                    ),
                    "average_anomaly_score": round(
                        analysis_stats.avg_anomaly_score or 0, 3
                    ),
                    "average_normalized_score": round(
                        analysis_stats.avg_normalized_score or 0, 1
                    ),
                    "average_confidence": round(analysis_stats.avg_confidence or 0, 3),
                },
                "attribution_breakdown": [
                    {
                        "attribution_type": attribution.anomaly_attribution
                        or "unknown",
            ...
            subject_distribution = (
                db.query(Drawing.subject, func.count(Drawing.id).label("count"))
                .group_by(Drawing.subject)
                .order_by(func.count(Drawing.id).desc())
                .all()
            )
            ...
            cross_subject_stats = (
                ...
                .group_by(Drawing.subject)
                .order_by(func.count(AnomalyAnalysis.id).desc())
                .all()
            )
            ...
            return {
                "overview": {
                    "total_subjects_with_data": len(
                        [s for s in subject_distribution if s.subject]
                    ),
                ...
                "subject_distribution": [
                    {
                        ...
                        "percentage": round(
                            subj.count
                            / sum(s.count for s in subject_distribution)
                            * 100,
                            1,
                        ),
                    }
                ...
                "cross_subject_analysis": [
                    {
                        ...
                        "anomaly_rate": round(
                            (stat.anomaly_count or 0)
                            / max(1, stat.total_analyses)
                            * 100,
                            1,
                        ),
                        "average_anomaly_score": round(stat.avg_anomaly_score or 0, 3),
                        "average_normalized_score": round(
                            stat.avg_normalized_score or 0, 1
                        ),
                    }
                ...
                "overall_attribution_breakdown": [
                    {
                        ...
                        "count": attr.count,
                        "percentage": round(
                            attr.count / max(1, total_analyses) * 100, 1
                        ),
                    }
...
async def get_subject_aware_model_status(db: Session = Depends(get_db)):
    ...
    try:
        ...
        active_models = (
            db.query(AgeGroupModel).filter(AgeGroupModel.is_active.is_(True)).all()
        )

        # Check subject-aware capabilities
        subject_aware_models = []
        for model in active_models:
            supports_subjects = getattr(
                model, "supports_subjects", True
            )  # Default to True for subject-aware system
            ...
            subject_aware_models.append(
                {
                    ...
                    "is_subject_aware": embedding_type == "hybrid"
                    and supports_subjects,
                }
            )

        # Calculate overall subject-aware status
        total_models = len(active_models)
        subject_aware_count = sum(
            1 for model in subject_aware_models if model["is_subject_aware"]
        )
        ...
        return {
            "subject_aware_system_status": {
                ...
                "coverage_percentage": round(
                    subject_aware_count / max(1, total_models) * 100, 1
                ),
            },
...
@router.post("/reset")
async def reset_system(confirm: bool = False, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        models = db.query(AgeGroupModel).filter(AgeGroupModel.is_active.is_(True)).all()
        ...
        logger.warning(
            f"System reset: deactivated {len(models)} models and cleared caches"
        )

        return SuccessResponse(
            message=f"System reset completed: {len(models)} models deactivated",
            data={"deactivated_models": len(models), "caches_cleared": True},
        )
