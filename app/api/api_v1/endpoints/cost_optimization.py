...
async def get_cost_estimate():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get cost estimate: {str(e)}"
        )
...
async def apply_s3_lifecycle_optimization(bucket_name: str):
    ...
    try:
        ...
        if success:
            return {
                "message": f"Lifecycle optimization applied to bucket: {bucket_name}"
            }
        else:
            raise HTTPException(
                status_code=500,
        ...
        raise HTTPException(
            status_code=500, detail=f"Error applying S3 optimization: {str(e)}"
        )
...
async def setup_cost_monitoring():
    ...
    try:
        ...
        if success:
            return {
                "message": "Cost monitoring and budget alerts configured successfully"
            }
        else:
            raise HTTPException(
                status_code=500, detail="Failed to set up cost monitoring"
            )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error setting up cost monitoring: {str(e)}"
        )
