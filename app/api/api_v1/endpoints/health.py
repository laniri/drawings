...
@router.get("/health/component/{component_name}", summary="Component-specific health check")
...
async def get_metrics_history(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Historical metrics retrieval failed: {str(e)}"
        )
...
async def update_alert_thresholds(thresholds: Dict[str, float]):
    ...
    try:
        ...
        valid_keys = ["cpu_percent", "memory_percent", "disk_percent", "response_time_ms"]
        ...
        for key, value in thresholds.items():
            if key.endswith("_percent") and not (0 <= value <= 100):
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Response time threshold must be positive"
                )
