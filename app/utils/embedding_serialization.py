...
class EmbeddingSerializer:
    ...
    @staticmethod
    def serialize_hybrid_embedding(
        ...
        try:
            if not isinstance(hybrid_embedding, np.ndarray):
                raise EmbeddingSerializationError(
                    f"Expected numpy array, got {type(hybrid_embedding)}"
                )
            ...
            full_bytes = pickle.dumps(
                hybrid_embedding, protocol=pickle.HIGHEST_PROTOCOL
            )
            visual_bytes = pickle.dumps(
                visual_component, protocol=pickle.HIGHEST_PROTOCOL
            )
            subject_bytes = pickle.dumps(
                subject_component, protocol=pickle.HIGHEST_PROTOCOL
            )

            logger.debug(
                f"Serialized hybrid embedding: full_size={len(full_bytes)}, "
            ...
            if isinstance(e, EmbeddingSerializationError):
                raise
            raise EmbeddingSerializationError(
                f"Failed to serialize hybrid embedding: {str(e)}"
            )

    @staticmethod
    def deserialize_hybrid_embedding(
        ...
        if full_bytes is not None:
            try:
                ...
                if isinstance(
                    hybrid_embedding, np.ndarray
                ) and hybrid_embedding.shape == (832,):
        ...
        if visual_bytes is not None and subject_bytes is not None:
            try:
                ...
                if (
                    isinstance(visual_component, np.ndarray)
    ...
    def validate_hybrid_embedding(embedding: np.ndarray) -> bool:
        ...
        try:
            ...
            if embedding.shape != (832,):
                logger.warning(
                    f"Invalid hybrid embedding shape: {embedding.shape} (expected (832,))"
                )
                return False
            ...
            if not (
                np.sum(subject_component) == 1.0
                and np.sum(subject_component == 1.0) == 1
            ):
                logger.warning(
                    "Invalid subject component in hybrid embedding (not one-hot)"
                )
                return False
    ...
    def serialize_embedding(embedding: np.ndarray) -> bytes:
        ...
        try:
            ...
            if len(embedding) == 0:
                raise EmbeddingSerializationError(
                    "Cannot serialize empty embedding array"
                )
            ...
            logger.debug(
                f"Serialized embedding: shape={embedding.shape}, dtype={embedding.dtype}, size={len(serialized_data)} bytes"
            )
            return serialized_data

        except Exception as e:
            raise EmbeddingSerializationError(
                f"Failed to serialize embedding: {str(e)}"
            )
    ...
    def deserialize_embedding(serialized_data: bytes) -> np.ndarray:
        ...
        try:
            if not isinstance(serialized_data, bytes):
                raise EmbeddingSerializationError(
                    f"Expected bytes, got {type(serialized_data)}"
                )
            ...
            if not isinstance(embedding, np.ndarray):
                raise EmbeddingSerializationError(
                    f"Deserialized data is not numpy array: {type(embedding)}"
                )
            ...
            logger.debug(
                f"Deserialized embedding: shape={embedding.shape}, dtype={embedding.dtype}"
            )
            return embedding

        except pickle.PickleError as e:
            raise EmbeddingSerializationError(
                f"Failed to deserialize embedding (pickle error): {str(e)}"
            )
        except Exception as e:
            raise EmbeddingSerializationError(
                f"Failed to deserialize embedding: {str(e)}"
            )

    @staticmethod
    def validate_embedding(
        embedding: np.ndarray, expected_dimension: Optional[int] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if embedding.ndim != 1:
                logger.warning(
                    f"Invalid embedding dimensions: {embedding.ndim} (expected 1)"
                )
                return False
            ...
            if expected_dimension is not None and len(embedding) != expected_dimension:
                logger.warning(
                    f"Invalid embedding size: {len(embedding)} (expected {expected_dimension})"
                )
                return False
...
class EmbeddingCache:
    ...
    def __init__(self, max_size: int = 1000, ttl_seconds: int = 3600):
        ...
        logger.info(
            f"EmbeddingCache initialized: max_size={max_size}, ttl={ttl_seconds}s"
        )

    def _generate_cache_key(
        self, drawing_id: int, model_type: str, age: Optional[float] = None
    ) -> str:
    ...
    def put(
        ...
        try:
            ...
            logger.debug(
                f"Cached embedding for key: {cache_key}, dimension: {len(embedding)}"
            )
...
class EmbeddingStorage:
    ...
    def store_hybrid_embedding(
        ...
        try:
            ...
            logger.debug(
                f"Stored hybrid embedding for drawing {drawing_id}: dimension={len(hybrid_embedding)}"
            )
        ...
        try:
            ...
            if use_cache:
                ...
                if cached_embedding is not None and cached_embedding.shape == (832,):
                    logger.debug(
                        f"Retrieved hybrid embedding from cache for drawing {drawing_id}"
                    )
            ...
            if hybrid_embedding is not None:
                ...
                logger.debug(
                    f"Retrieved hybrid embedding from database for drawing {drawing_id}"
                )
                return hybrid_embedding
            ...
            raise EmbeddingSerializationError(
                f"Failed to retrieve hybrid embedding: {str(e)}"
            )
    ...
    def store_embedding(
        ...
        try:
            ...
            logger.debug(
                f"Stored embedding for drawing {drawing_id}: dimension={len(embedding)}"
            )
        ...
        try:
            ...
            if use_cache:
                ...
                if cached_embedding is not None:
                    logger.debug(
                        f"Retrieved embedding from cache for drawing {drawing_id}"
                    )
                    return cached_embedding
            ...
            if serialized_data is not None:
                ...
                logger.debug(
                    f"Retrieved embedding from database for drawing {drawing_id}"
                )
                return embedding
    ...
    def batch_store_embeddings(
        self, embeddings_data: List[Dict[str, Any]], use_cache: bool = True
        ...
        for data in embeddings_data:
            try:
                ...
                logger.error(
                    f"Failed to store embedding for drawing {data.get('drawing_id')}: {str(e)}"
                )
...
def serialize_hybrid_embedding_for_db(
    hybrid_embedding: np.ndarray,
) -> Tuple[bytes, bytes, bytes]:
    """
...
def deserialize_hybrid_embedding_from_db(
    full_bytes: Optional[bytes] = None,
    visual_bytes: Optional[bytes] = None,
    subject_bytes: Optional[bytes] = None,
) -> Optional[np.ndarray]:
    ...
    Returns:
        832-dimensional hybrid embedding or None if reconstruction fails
    """
    return EmbeddingSerializer.deserialize_hybrid_embedding(
        full_bytes, visual_bytes, subject_bytes
    )
